...
logging.basicConfig(filename='crawler_error.log', level=logging.DEBUG)
...
class Crawler(object):
    ...
    def __init__(self, max_threads):
        ...
        self.file_object = pymongo_recorder(self.max_threads)
            ...
            if self.file_object.record_url(queue_item[0]):
                ...
                for url in self.urls:
                    if queue_item[1] + 1 <= 2 and (not (url in self.file_object.scanned_urls)):
                        ...
                        pprint(url)
                        self.queue.put((complete_domain(url, self.current_url) ,
                                        queue_item[1] + 1)) # Add the url to the queue and increase the depth
